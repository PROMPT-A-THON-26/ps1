(() => {
  ...
  function renderSummary(){
    ...
    const nodeHealth=nodes.length?Math.round((healthy/nodes.length)*100):0;
    const replicaStats=DATA.objects.map(o=>{const m=String(o.replicas||"").match(/^(\d+)\/(\d+)$/);return m?{healthy:Number(m[1]),total:Number(m[2])}:null;}).filter(Boolean);
  ...
  function renderDetail(o){
    ...
    $("#object-detail").innerHTML="<div class='detail-grid'><div><div class='detail-hero'><p class='eyebrow'>FILE DETAILS</p><h2>"+escapeHtml(o.name||o.id)+"</h2><div class='detail-meta'><span>"+escapeHtml(o.size)+"</span><span>Version "+escapeHtml(o.version)+"</span><span>"+status(o.status)+"</span><span>SHA-256 "+escapeHtml(o.checksum)+"</span></div>"+actionButtons+"</div><div style='padding-top:12px'><p class='eyebrow'>PROTECTED COPIES</p><div class='replicas'>"+replicaCards+"</div></div></div><div><p class='eyebrow'>METADATA</p><div class='service-list'><div><span>Created</span><b>"+escapeHtml(o.created)+"</b></div><div><span>Version ID</span><b>"+escapeHtml(o.currentVersionId||"—")+"</b></div><div><span>Version number</span><b>"+escapeHtml(liveVersion.version_number?("v"+liveVersion.version_number):o.version)+"</b></div><div><span>Healthy replicas</span><b>"+escapeHtml(o.replicas)+"</b></div><div><span>Placement policy</span><b>"+escapeHtml(placementPolicy)+"</b></div></div></div></div>";
  }
