...
from common.errors import ChecksumMismatch, InvalidState, ObjectAlreadyExists, ObjectNotFound, VersionConflict
...
class MetadataManager:
    ...
    def get_object(self, name: str) -> Optional[Object]:
        # Keep read-only calls transaction-scoped so they do not leave an
        # implicit/autobegun transaction open for the next write operation.
    ...
    def create_object(self, name: str) -> Object:
        with self.session.begin():
            existing = self.session.scalar(
                select(Object).where(Object.name == name).with_for_update()
            )
            if existing is not None:
                raise ObjectAlreadyExists(name)
            obj = Object(name=name, state=ObjectState.ACTIVE)
    ...
    def _current_version_number(self, obj: Object) -> Optional[int]:
        ...
        return None if current is None else current.version_number
    ...
    def create_version(
        ...
        *,
        ...
        if any(ch not in "0123456789abcdef" for ch in normalized_checksum):
        ...
        with self.session.begin():
            ...
            max_version = self.session.scalar(
                select(func.max(Version.version_number)).where(Version.object_id == object_id)
            )
        ...
        *,
        ...
        with self.session.begin():
            ...
            if current_number is not None and version.version_number <= current_number:
                raise VersionConflict(current_number, current_number)
    ...
    def set_replica_state(self, replica_id: UUID, state: ReplicaState) -> Replica:
        with self.session.begin():
            ...
            if state not in allowed.get(replica.status, set()):
                raise InvalidState(f"Cannot transition replica from {replica.status} to {state}.")
        ...
        with self.session.begin():
            ...
            if existing is not None:
                if node_id is not None and existing.node_id != node_id:
                    raise ObjectAlreadyExists(address)
                return existing

            node = StorageNode(
                node_id=node_id or new_uuid().hex,
                address=address,
