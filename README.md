...
The Compose stack keeps PostgreSQL, Redis, and storage nodes off host-published ports. Application containers use non-root users, dropped Linux capabilities, `no-new-privileges`, a read-only root filesystem, and a hardened writable `/tmp` mount.

Database and Redis credentials are runtime inputs rather than committed fixed passwords. For CI/e2e, ephemeral credentials are generated by the workflow. For local development, copy the values from `.env.example` into a local `.env` and replace placeholders with strong values.

The frontend remains safe to host as a static artifact: hosted pages default to deterministic demo mode; live API access requires an explicit `?mode=api` selection and a configured API base URL.
